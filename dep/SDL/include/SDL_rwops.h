 ...
 *  data streams.  It can easily be extended to files, m_memory, etc.
 */
...
#define SDL_RWOPS_MEMORY_RO 5U  /**< Read-Only m_memory stream */
#if defined(__VITA__)
...
/**
 * Use this function to prepare a read-write m_memory buffer for use with
 * SDL_RWops.
 *
 * This function sets up an SDL_RWops struct based on a m_memory area of a
 * certain size, for both read and write access.
 *
 * This m_memory buffer is not copied by the RWops; the pointer you provide must
 * remain valid until you close the stream. Closing the stream will not free
 * the original buffer.
 *
 * If you need to make sure the RWops never writes to the m_memory buffer, you
 * should use SDL_RWFromConstMem() with a read-only buffer of m_memory instead.
 *
...
/**
 * Use this function to prepare a read-only m_memory buffer for use with RWops.
 *
 * This function sets up an SDL_RWops struct based on a m_memory area of a
 * certain size. It assumes the m_memory area is not writable.
 *
 * Attempting to write to this RWops stream will report an error without
 * writing to the m_memory buffer.
 *
 * This m_memory buffer is not copied by the RWops; the pointer you provide must
 * remain valid until you close the stream. Closing the stream will not free
 * the original buffer.
 *
 * If you need to write to a m_memory buffer, you should use SDL_RWFromMem()
 * with a writable buffer of m_memory instead.
 *
 ...
 *
 * \returns a pointer to the allocated m_memory on success, or NULL on failure;
 ...
 * invalid as soon as this function returns. Any extra m_memory allocated during
 * creation of the SDL_RWops is not freed by SDL_FreeRW(); the programmer must
 * be responsible for managing that m_memory in their **close** method.
 *
