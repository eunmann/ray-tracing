...
typedef struct SDL_RendererInfo
{
    ...
    int max_texture_width;      /**< The maximum texture m_width */
    int max_texture_height;     /**< The maximum texture m_height */
} SDL_RendererInfo;
 ...
 *
 * \param width the m_width of the window
 * \param height the m_height of the window
 ...
 *
 * \param renderer the rendering context
 * \param w an int filled with the m_width
 * \param h an int filled with the m_height
 * \returns 0 on success or a negative error code on failure; call
 *          SDL_GetError() for more information.
 *
 * \since This function is available since SDL 2.0.0.
 ...
 *          was active, the format was unsupported, or the m_width or m_height
...
/**
 * Unlock a texture, uploading the changes to video m_memory, if needed.
 *
 * **Warning**: Please note that SDL_LockTexture() is intended to be
 * write-only; it will not guarantee the previous contents of the texture will
 * be provided. You must fully initialize any area of a texture that you lock
 * before unlocking it, as the pixels might otherwise be uninitialized m_memory.
 *
 ...
 * directly.
 *
 * If not NULL, `texw` and `texh` will be filled with the m_width and m_height
 * values suitable for the provided texture. In most cases, both will be 1.0,
 * however, on systems that support the GL_ARB_texture_rectangle extension,
 * these values will actually be the pixel m_width and m_height used to create the
 ...
 *             texture m_width or NULL if you don't need that value
 * \param texh a pointer to a float value which will be filled with the
 *             texture m_height or NULL if you don't need that value
